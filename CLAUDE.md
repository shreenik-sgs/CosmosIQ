...
- **Part I — Foundation is frozen (ADR-0007).** Chapters `EIOS-000`…`EIOS-006` are Foundation Release 2.0 and architecturally stable: do not modify them with manuscript patches — changes require a new ADR with review. The freeze covers those chapters only; the Architecture Book version still increments, and appendices/reference material (Architecture Registry, Architectural Dictionary, Graph Taxonomy, etc.) and future Parts (II–V, EIOS-007+) continue to evolve freely. `EIOS-000` additionally remains under ADR-0006.
- **A chapter may be authored across multiple patches.** One chapter can be built from several manuscript increments (e.g. EIOS-004 via `AB-0004A` + `AB-0004B`). Later increments **extend** the existing chapter rather than creating a second chapter: keep one `BEGIN/END:CHAPTER` pair and one canonical title (no "Part I/II" suffixes). Slot the new narrative sections into the body, but **merge** the shared tail sections (Architectural Rules, Requirements Introduced, Future Dependencies, Cross References) into a single instance of each — never duplicate them — keeping Cross References last. Regenerate once the chapter/batch is complete.
