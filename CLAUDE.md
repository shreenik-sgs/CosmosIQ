...
- **A chapter may be authored across multiple patches.** One chapter can be built from several manuscript increments (e.g. EIOS-004 via `AB-0004A` + `AB-0004B`). Later increments **extend** the existing chapter — insert their content before its single `<!-- END:CHAPTER:ID -->` marker — rather than creating a second chapter. Keep one `BEGIN/END:CHAPTER` pair and one canonical title (no "Part I/II" suffixes); regenerate once the chapter/batch is complete.
