...
4. User Confirmation  ↔  what the user placed — matched by **`broker_order_id`**
5. what the user placed (**`broker_order_id`**)  ↔  **broker acknowledgment / record**
6. broker acknowledgment  ↔  fills recorded
7. fills  ↔  resulting position
8. resulting position  ↔  expected position — **actual capital deployed vs `intended_allocation`**
...
## Authoritative source by cadence
- **Intraday checks** (immediately after a placement or fill): the **live broker position** is authoritative.
- **Final reconciliation** (end of session / day): the **end-of-day broker statement** is authoritative.
